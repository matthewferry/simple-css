...
const rootSize = textConfig.rootSize;
const gridUnit = textConfig.gridUnit;
...
textConfig.sizes.forEach((size) => {
  const fontSize = scaleBaseSize * Math.pow(scaleRatio, size.scale); // Returns pixel size
  const lineHeight = size.hasOwnProperty("lineHeight") ? (
    Math.round(size.lineHeight * fontSize / rootSize * 2) / 2 // Returns rem unit
  ) : (
...
fs.writeFile('src/theme/_text.css', ':root {' + variableSheet + '\n}\n', () => true);
